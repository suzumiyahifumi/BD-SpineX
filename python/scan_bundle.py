...
def main():
    ...
    try:
        ...
        print(json.dumps({"ok": False, "error": f"UnityPy import failed: {error}", "elapsedMs": elapsed_ms(started)}))
        return 0
        ...
        for obj in env.objects:
            ...
            if type_name == "Texture2D":
                ...
                if texture_format is not None:
                    asset["textureFormat"] = int(texture_format)
                    asset["textureFormatName"] = str(texture_format)
                stream_data = getattr(data, "m_StreamData", None)
                stream_size = getattr(stream_data, "size", None)
                stream_path = getattr(stream_data, "path", None)
                if stream_size is not None:
                    asset["streamDataSize"] = int(stream_size)
                if stream_path:
                    asset["streamDataPath"] = str(stream_path)

            assets.append(asset)

        assets.sort(key=lambda asset: (asset["name"].lower(), asset["type"]))
        print(json.dumps({"ok": True, "assets": assets, "skipped": skipped, "elapsedMs": elapsed_ms(started)}, ensure_ascii=False))
        return 0
    except Exception as error:
        print(json.dumps({"ok": False, "error": str(error), "elapsedMs": elapsed_ms(started)}, ensure_ascii=False))
        return 0
