...
def main():
    ...
    timings = []
    parser = argparse.ArgumentParser(description="Patch Unity AssetBundle assets for BD-SpineX mods with UnityPy.")
...
def find_missing(replacements, changed):
    ...
    for name, replacement in replacements["text"].items():
        if ("TextAsset", name) not in changed_keys:
            missing.append({"type": "TextAsset", "name": name, "source": replacement["path"], "modName": replacement.get("modName")})

    for name, replacement in replacements["texture"].items():
        if ("Texture2D", name) not in changed_keys:
            missing.append({"type": "Texture2D", "name": name, "source": replacement["path"], "modName": replacement.get("modName")})
